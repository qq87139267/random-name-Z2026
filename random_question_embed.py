...
class App:
    ...
    def setup_ui(self):
        self.label_class = tk.Label(self.root, text="", font=("微软雅黑", 40, "bold"), fg="blue", bg="#1a1a2e")
        self.label_class.pack(pady=20)
        
        self.label_name = tk.Label(self.root, text="准备", font=("华文行楷", 120, "bold"), fg="#ff4757", bg="#1a1a2e")
        self.label_name.pack(expand=True)
        
        self.label_remain = tk.Label(self.root, text="", font=("微软雅黑", 30), fg="white", bg="#1a1a2e")
        self.label_remain.pack(pady=10)
        
        btn_frame = tk.Frame(self.root, bg="#1a1a2e")
        btn_frame.pack(pady=20)
        
        tk.Button(btn_frame, text="开始/停止", command=self.toggle_roll, font=("微软雅黑", 20), width=10, bg="#1e90ff", fg="white").pack(side=tk.LEFT, padx=10)
        tk.Button(btn_frame, text="重置", command=self.reset, font=("微软雅黑", 20), width=10).pack(side=tk.LEFT, padx=10)
        tk.Button(btn_frame, text="换班", command=self.switch_class, font=("微软雅黑", 20), width=10).pack(side=tk.LEFT, padx=10)
    ...
    def update_display(self):
        ...
        if not self.is_rolling:
            self.label_name.config(text="按空格" if len(self.remaining_names)>0 else "已抽完")
